...
#output torch batch*3 z, x, y in radiant
#the rotation is in the sequence of z,x,y
def compute_euler_angles_from_rotation_matrices(rotation_matrices):
    ...
    x = torch.atan2(R[:,2,1], sy)
    y = torch.atan2(-R[:,0,2], R[:,2,2])
    
    zs = torch.atan2(-R[:,0,1], R[:,0,0])
    xs = torch.atan2(R[:,2,1], sy)
    ys = R[:,0,2]*0
    ...
    out_euler[:,1] = x*(1-singular)+xs*singular
    out_euler[:,2] = y*(1-singular)+ys*singular
